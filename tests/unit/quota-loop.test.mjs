...
test('records left in the old shape are shrunk in place', () => {
  ...
  assert.ok(freed.bytes > before * 0.3, `should free roughly the base copy, freed ${freed.bytes} of ${before}`);
  const meta = JSON.parse(storage.getItem('vmx-user-sync-v1:user-1'));
  assert.ok(!('base' in meta.dirty.history), 'base is gone');
  assert.deepEqual(meta.dirty.history.value, grown, 'the value is carried over untouched');
  assert.deepEqual(meta.dirty.history.added, ['history:2026-09-14T11:00:00.000Z:com3:999']);
...
test('compaction runs at boot, so a returning full device gets room without a write succeeding', () => {
  ...
  assert.ok(!('base' in meta.dirty.history));
});
