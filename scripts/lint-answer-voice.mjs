...
for (const f of banks) {
  ...
  for (const q of entry[1]) {
    ...
    if (!text) continue;
    scanned++;
    const broke = RULES.filter(([, re]) => re.test(text));
    if (!broke.length) continue;
    hits.push({
      id: q.id, file: f, subject: q.subject, topic: q.topic,
      rules: broke.map(([k]) => k),
      why: broke.map(([, , w]) => w),
      quote: broke.map(([, re]) => { const i = text.search(re); return text.slice(Math.max(0, i - 45), i + 95).replace(/\s+/g, ' '); }),
      explain: text,
    });
  }
}
...
for (const [r, n] of Object.entries(byRule).sort((a, b) => b[1] - a[1])) {
  console.log(`  ${String(n).padStart(4)}  ${r.padEnd(14)} ${RULES.find(([k]) => k === r)[2]}`);
}
