...
## 2026-09-26 — Security 5.130.16 released (`69e7f158`)
...
- Live password reauthentication enabled/read-back true; HIBP enable returned402(Pro required), unchanged. Security was released separately: exact-SHA Build `36249430184`, Smoke `36249430130` and Production deployment `6680280408` are successful. The security lane's live app/PDF/cache and Archive Worker receipts are in the primary checkout's `work/security-20260926/HANDOFF.md`; authenticated production-account and physical-device paths remain outside that proof. Do not replay its SQL or rotate the MAC key.
...
## 2026-09-26 — Learning upgrade 5.131.0 released (`3e6fb3eb`)

- Branch `codex/learning-upgrade-0926`, original base `9fb038d5`. Details and integration seams: `docs/learning-upgrade-2026-09-26.md`; local evidence and source hash manifest: `work/astra-upgrade-20260926/`.
- Reuses `buildExamPool` for truthful planned counts and shared `reviewQuestionsInContext` for Home/SR paper scope. Home-only statistics include personal cards; Home launches open their advertised filters. Plans retain the time budget, prioritize an imminent paper, and explain the reading reserve. Native subject selection keeps mobile review controls close together.
- Shared idle imports respect offline/Save-Data at execution; a failed decorative loader stays inside `OptionalFeature`. Existing preference storage is reused; no new dependency, clinical content, schema or forced update lifecycle.
- Final integrated checks: all 51 data/unit gate steps pass (full units in both timezones), build, dependency audit and three contrast audits pass. CI-parity E2E: Chromium 372 passed/18 skipped; WebKit+Firefox 365 passed/1 flaky/24 skipped, both exit 0. The unchanged landing scroll test passed retry and 3/3 isolated no-retry follow-ups; the original-base Pomodoro flake also passed 3/3 isolated. Do not describe this as zero-flake.
- Inspect targeted screenshots before the final broad gate: an overflow pass alone did not catch the long SR subject list. The native selector is checked for subject switching and a start control below 1150px on a 320px viewport.
- Released as `3e6fb3eb11bc8f71e3a0ab450805f2ace54bb899`, 5.131.0/SWv196, on security `69e7f158`. Exact-SHA Build `36251964739`, Smoke `36251964730`, Vercel `FR2d3T2AxkDs7R4t3uUjePxT7oiy` / Production `6680766321` succeeded. Live changed-flow tests passed 12/12, no retries. An existing SW195 feedback draft kept its text/document/controller; a separate fresh context verified the full SW196 source. Immediate reopen in the draft context remained on SW195, so do not promise immediate activation. Proof: `work/astra-upgrade-20260926/RELEASE-STATUS.md` in the release worktree and primary checkout.

## 2026-09-26 — Study journey continuity candidate (5.132.0)

- Continues the authorized upgrade/release loop on `codex/study-loop-0926`, based on deployed `3e6fb3eb`. Worktree reused; no primary checkout reset. Scope and integration: `docs/study-loop-2026-09-26.md`; evidence: `work/study-loop-0926/`.
- Dashboard handles undated/legacy-date histories and uses curriculum year mapping before lazy banks load. Checklist projects canonical topic scope plus semester without erasing marks, and surfaces rejected writes. Schedule uses real per-paper availability, selects a named paper's phase, clears stale topics, and opens Notes where practice is unavailable.
- App/Notes use native history entries for the saved subject/topic and origin. Checklist/calendar scroll waits for the view chunk; Wiki returns to the original article heading. The view-entry focus guard respects open modals. No new router, store, dependency, content, schema or worker takeover.
- Browser tests cover persisted exam questions, accepted undated backup, failed Notes download/reload, slow return chunks, Back/Forward and search focus. Read local RELEASE-STATUS for final gates and exact deployment proof; a candidate/build alone is not production.
- Preserve bughunt App/Notes/engine hunks and sync/content/Research Studio worktrees. User authorized production and agent notification; prepare one coordinated push after gates, retain security `69e7f158`, and do not replay migrations or upload the Archive Worker.
