...
3. กด "สมัคร"
4. (Optional) เช็ค email → กด link verify
   > ⚠️ **แนะนำให้ปิด email confirmation** เพื่อหลีกเลี่ยงปัญหา email ไม่มา:
   > Supabase Dashboard → Authentication → Sign In / Providers → Email → **uncheck "Confirm email"**

> 💡 ถ้าปิด email confirmation: signup เสร็จ → login ได้เลย ไม่ต้อง verify
...
### Email verify ไม่มา
- **แนะนำ:** ปิด email confirmation ใน Supabase → Authentication → Sign In / Providers → Email → uncheck "Confirm email"
- หรือเข้า Supabase → Authentication → Users → กด verify manual ได้
- หรือตั้ง custom SMTP (Gmail) ใน Project Settings → Auth
