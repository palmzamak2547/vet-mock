-- Security hardening for VetMock's browser-facing Supabase surface.
--
-- The migration is intentionally paired with the frontend release that switches
-- group creation/joining to the two RPCs below. Apply and deploy them together:
-- removing the old direct INSERT policies before the client uses the RPCs would
-- make the existing group UI fail closed.

-- ---------------------------------------------------------------------------
-- 1. Least-privilege baseline for browser roles
-- ---------------------------------------------------------------------------

-- PostgREST only needs SELECT/INSERT/UPDATE/DELETE. These object-management
-- privileges were inherited from the project's old broad defaults and must
-- never be available to browser roles.
revoke truncate, references, trigger on all tables in schema public
  from anon, authenticated;

-- Unauthenticated visitors only read public data. All anonymous mutations in
-- VetMock go through narrowly scoped RPCs (for example daily_q_pulse).
...
-- ---------------------------------------------------------------------------
-- 2. Stop column leaks that row-level policies cannot prevent
-- ---------------------------------------------------------------------------

-- profiles is a public directory, but study metrics are private. RLS limits
-- rows; it cannot hide total_attempts/total_correct/streak/last_study_date.
revoke select on table public.profiles from anon, authenticated;
grant select (id, username, avatar_emoji, created_at)
  on table public.profiles to anon, authenticated;

-- Users may edit only their public display fields. Authorization fields do not
-- live here, but column grants keep future metrics from becoming self-writable.
revoke update on table public.profiles from anon, authenticated;
grant update (username, avatar_emoji)
  on table public.profiles to authenticated;

-- Profiles are created by the auth trigger or ensure_profile(), both running
-- with server authority. Direct INSERT would let a caller populate future
-- metric columns before those columns are protected by application logic.
...
-- Public Imaging Practical cards need the descriptive fields below, never the
-- private answer key, consent audit bit, or creator UUID.
...
-- The bucket is private, but its old SELECT policy allowed every object in the
-- bucket. Tie Storage authorization to the public case registry so draft or
-- future private cases cannot be listed or signed by browser clients.
...
-- ---------------------------------------------------------------------------
-- 3. Make invite codes the only way to join a study group
-- ---------------------------------------------------------------------------

-- The former groups_select_by_code policy used USING (true), exposing every
-- invite code. The former group_members INSERT policy also let a caller join
-- any known UUID and choose role='admin'. Remove both direct paths.
...
revoke insert, update on table public.groups from anon, authenticated;
revoke all privileges on table public.group_members from anon;
...
-- Keep recursive membership checks out of the exposed public RPC namespace.
...
grant execute on function private.is_current_user_group_member(uuid)
  to authenticated;
...
create policy "sq_select_group_members"
...
create policy "sq_insert_member"
  on public.shared_questions
  for insert
  to authenticated
  with check (
...
-- record_daily_q_pulse(boolean) and seeside_public_stats() intentionally stay
-- anonymous: the former is the privacy-preserving Daily Q aggregate and the
-- latter returns only public counts. Do not revoke them as collateral damage.
