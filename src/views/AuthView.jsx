      ...
      <div className="vmx-config-panel" style={{ maxWidth: 440, margin: '0 auto' }}>
        ...
        {mode !== 'reset' && mode !== 'update-password' && mode !== 'magic-link' && (
          <>
            ...
            {/* Apple Sign-in — for iOS PWA users. Same config-not-set
                fallback as LINE. */}
            <button
              type="button"
              className="vmx-btn vmx-btn-ghost"
              style={{ width: '100%', justifyContent: 'center', padding: '14px', marginBottom: 8, background: '#000', color: '#fff', borderColor: '#000' }}
              onClick={appleLogin}
              disabled={loading}
            >
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontWeight: 600 }}>
