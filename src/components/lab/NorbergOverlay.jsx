...
export default function NorbergOverlay({ active, viewportRef, caseId = null }) {
  ...
  const exportStateJson = useCallback(() => {
    ...
    // Export the measured points as a portable record for later analysis.
    const data = {
      ...
      {angles && (
        <div style={isMobile ? (cardCollapsed ? mobileSheetCollapsedStyle : mobileSheetStyle) : resultCardStyle}>
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
            ...
            <button type="button" onClick={exportStateJson} className="vmx-tap" style={resetBtnStyle} title="ดาวน์โหลด JSON ของ Norberg points เพื่อเก็บผลการวัด">
              JSON
            </button>
            <button
              type="button"
